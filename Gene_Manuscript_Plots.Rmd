...
# Prep read count table and switch to long format
mnorm$Genes <- rownames(mnorm)
mnorm <- mnorm[which(mnorm$Genes %in% spark_key$Gene), ]
colnames(mnorm) <- gsub(".nonrRNA", "", colnames(mnorm))
mnorm <- melt(mnorm)
# Add columns of timepoint and phylum
mnorm$Timepoint <- metadata$Timepoint[match(mnorm$variable, metadata$Sample)]
mnorm$Taxonomy <- spark_key$Phylum[match(mnorm$Genes, spark_key$Gene)]

# Aggregate by phylum
spark_phyla <- aggregate(value ~ Taxonomy, data = mnorm, mean)
...
# Prep read count table and switch to long format
mnorm$Genes <- rownames(mnorm)
mnorm <- mnorm[which(mnorm$Genes %in% trout_key$Gene), ]
colnames(mnorm) <- gsub(".nonrRNA", "", colnames(mnorm))
mnorm <- melt(mnorm)
# Add columns of timepoint and phylum
mnorm$Timepoint <- metadata$Timepoint[match(mnorm$variable, metadata$Sample)]
mnorm$Taxonomy <- trout_key$Phylum[match(mnorm$Genes, trout_key$Gene)]

# Aggregate by phylum
trout_phyla <- aggregate(value ~ Taxonomy, data = mnorm, mean)
...
within_reps <- function(readcounts){
  readcounts2 <- readcounts[which(rowSums(readcounts) > 10000), ]
