...
# mendota_metaG_phyla <- read.csv(file = paste(path2, "Desktop/geodes/intermediate_plotting_files/mendota_metaG_phyla.csv", sep = ""), header = T)
# spark_metaG_phyla <- read.csv(file = paste(path2, "Desktop/geodes/intermediate_plotting_files/spark_metaG_phyla.csv", sep = ""), header = T)
# trout_metaG_phyla <- read.csv(file = paste(path2, "Desktop/geodes/intermediate_plotting_files/trout_metaG_phyla.csv", sep = ""), header = T)
# 
# 
...
# Read back in
mendota_phyla <- read.csv(file = paste(path2, "Desktop/geodes/intermediate_plotting_files/mendota_metaT_phyla.csv", sep = ""), header = T)
...
# Read back in
spark_phyla <- read.csv(file = paste(path2, "Desktop/geodes/intermediate_plotting_files/spark_metaT_phyla.csv", sep = ""), header = T)
...
# Read back in
trout_phyla <- read.csv(file = paste(path2, "Desktop/geodes/intermediate_plotting_files/trout_metaT_phyla.csv", sep = ""), header = T)
...
# note to self: change graphics size
...
ggplot(mendota_cv_stats, aes(stat, fill = type)) + geom_density(alpha = 0.5) + scale_fill_manual(values = c("#d8b365", "#5ab4ac")) + labs(x = "Coefficent of Variance (%)", title = "Lake Mendota") + theme(legend.title = element_blank())
...
example <- data.frame("time" = agg_mnorm$Time[which(agg_mnorm$Genes == genes[1])], "eigenvector" = zscore(agg_mnorm$value[which(agg_mnorm$Genes == genes[1])]))

ggplot(data = example, aes(x = time, y = eigenvector)) + geom_bar(stat = "identity")
```
...
```{r, eval = T, echo = F, error = F, mesage = F}
# spark_within <- within_reps(snorm)
# spark_within <- apply(spark_within, 1, mean, na.rm = T)
# spark_across <- apply(snorm, 1, cv, na.rm = T)
# cv_stat <- c(rep("within replicates", length(spark_within)), rep("across samples", length(spark_across)))
...
ggplot(trout_cv_stats, aes(x = stat, fill = type)) + geom_density(alpha = 0.5) + scale_fill_manual(values = c("#d8b365", "#5ab4ac")) + labs(x = "Coefficent of Variance (%)", title = "Trout Bog") + theme(legend.title = element_blank())
```
