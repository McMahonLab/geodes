...
```{r, make_genome_table, echo = T, warning = F, message = F}
...
```{r, plot_mapped, echo = T, fig.height = 10, fig.width = 8, warning = F, message = F}
...
```{r, kraken, echo = T, warning = F, message = F}
...
samples <- list.files(path2results)

for (i in 1:length(samples)){
  results <- read.table(paste(path2results, samples[i], sep = ""), header = F, sep = "\t")
...
order_data <- GEODES001_nonrRNA.report[which(GEODES001_nonrRNA.report$Rank_code == "O" & GEODES001_nonrRNA.report$Percent_reads > 0), c(1, 6)]
...
num_ME <- length(which(samplenames$Lake == "Mendota"))
num_TB <- length(which(samplenames$Lake == "Trout"))
num_SP <- length(which(samplenames$Lake == "Sparkling"))

agg_class$Percent_reads[which(agg_class$Lake == "Mendota")] <- agg_class$Percent_reads[which(agg_class$Lake == "Mendota")]/num_ME
agg_class$Percent_reads[which(agg_class$Lake == "Trout")] <- agg_class$Percent_reads[which(agg_class$Lake == "Trout")]/num_TB
...
agg_class <- agg_class[order(agg_class$Percent_reads, decreasing = T),]
print(agg_class)
```
