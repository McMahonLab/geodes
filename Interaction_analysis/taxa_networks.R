...
keep <- c()
for(i in 1:length(taxa)){
  search <- paste(taxa[i], "$", sep = "")
  numgenes <- length(grep(search, mendota_key$Taxonomy))
  if(numgenes < 100){
    keep[i] <- FALSE
  }else{
    keep[i] <- TRUE
...
write.csv(vert.data, file = "C:/Users/Goose and Gander/Desktop/geodes/Interaction_analysis/ME_cluster_data.csv", quote = F, row.names = T)
...
vert.data[which(vert.data$Cluster == c), ]


#Output files for grepping genes
#Note: I'm combining some taxa that are classified at different resolutions but likely the same pop. based on high correlations. For example, Bacteroidetes;Chitinophagia;Chitinophagales;and Bacteroidetes;Chitinophagia;Chitinophagales;Chitinophagaceae; are getting lumped together into 1 fasta file called Chitinophagaceae
genes2grep <- data.frame(mendota_key$Gene[grep("Verrucomicrobia;Verrucomicrobiae;Verrucomicrobiales;Verrucomicrobiaceae;Prosthecobacter;Prosthecobacter debontii;Prosthecobacter debontii ATCC 700200;$", mendota_key$Taxonomy)])
write.table(genes2grep, "C:/Users/Goose and Gander/Desktop/geodes/Interaction_analysis/Gene_lists/ME13_Prosthecobacter_debontii.txt", row.names = F, quote = F)
...
for(i in 1:length(taxa)){
  search <- paste(taxa[i], "$", sep = "")
  numgenes <- length(grep(search, spark_key$Taxonomy))
  if(numgenes < 100){
    keep[i] <- FALSE
  }else{
    keep[i] <- TRUE
...
#Output files for grepping genes
# Similar to Mendota where I'm combining groups that match taxonomies with one more classified than the other, I'm also combining groups closely related + classified to a high level of resolution (ie I'm not convinced that strongly correlated groups acI-A1, acI-A4, and acI-A6 are truly different pops, so I combine them and call them acI-A)
# (this wasn't an issue in any of the Mendota clusters)
genes2grep <- data.frame(spark_key$Gene[grep("Proteobacteria;Alphaproteobacteria;Rhodospirillales;Acetobacteraceae;$", spark_key$Taxonomy)])
write.table(genes2grep, "C:/Users/Goose and Gander/Desktop/geodes/Interaction_analysis/Gene_lists/SP9_Acetobacteraceae.txt", row.names = F, quote = F)
...
for(i in 1:length(taxa)){
  search <- paste(taxa[i], "$", sep = "")
  numgenes <- length(grep(search, trout_key$Taxonomy))
  if(numgenes < 100){
    keep[i] <- FALSE
  }else{
    keep[i] <- TRUE
...
#Output files for grepping genes
genes2grep <- data.frame(trout_key$Gene[grep("Verrucomicrobia;Verrucomicrobiae;Verrucomicrobiales;Verrucomicrobia subdivision 3;$", trout_key$Taxonomy)])
write.table(genes2grep, "C:/Users/Goose and Gander/Desktop/geodes/Interaction_analysis/Gene_lists/TB8_Verrucomicrobia_subdivision3.txt", row.names = F, quote = F)
