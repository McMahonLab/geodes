...
Still failed runs - looks like some have "codon =" in the 2nd postion, locus tag in the third. Others are less clear. Is there a way I can split up the columns and keep only the one that says locus tag, no matter what column it's in?

```{bash, eval = F}
head -15 metaG_gffs/GEODES0050039 > test.txt
awk -F'\t' -v OFS='\t' '{print $9}' test.txt > tags.txt
awk -F ";" '{print $1,$2,$3,$4,$5,$6,$7,$8}' tags.txt > splittags.txt

#or
awk -F";" '{for(i=1;i<=NF;i++){if ($i ~ /locus_tag=*/){print $i}}}' tags.txt

#2nd one works!
```

CRISPR strikes again! Add the grep CDS line from the metagenome assembly to the other two.


####2017-09-14

Looks like about 1/3 of metaG pieces failed :( Here's how I'm checking:
```{bash, eval = F}
#List empty files
find CDS_GEODES*fna -type f -size 0 | wc -l

#list non empty files
find CDS_GEODES*fna -type f ! -size 0 | wc -l

#move non empty files into the results folder
find CDS_GEODES*fna -type f ! -size 0 | while read line; do mv $line CDS_fastas/; done

#make a list of emtpy files
ls CDS_GEODES*fna  > tryagain_metaGs.txt
sed -i 's/CDS_//' tryagain_metaGs.txt
sed -i 's/.fna//' tryagain_metaGs.txt

```

Modify the submit file for this. I've only looked through a handful of the +1000 error messages, but they seem to be a) 32 bit instead of 64 bit system b) ran out of RAM c) I have no idea. But I can address the first two, at least. Added "(Arch == "X86_64")" to the submit, upped the specs to 4GB hard drive and 16GB RAM. I also think there may be an issue with programs continually writing output over each other in gluster - I'll have them each copy the fasta file over instead. Zipping for the transfer.

####2017-09-15
Looks like 1/5 of test files worked? What are the errors of the other 4?
- gt extractfeat: error: no description matched sequence ID 'Ga0164295_10083215'
- gt extractfeat: error: no description matched sequence ID 'Ga0164295_10085076'
- gt extractfeat: error: no description matched sequence ID 'Ga0164295_10086941'
- gt extractfeat: error: no description matched sequence ID 'Ga0164295_10088832'

From github, the code that produces this error is:
...
Still unclear. Are those gene names in the gff files or the fasta?
They are in the gff files, but missing from the fasta file. That is odd. Maybe the file got truncated in downloading? Check the lakes drive version. Checksum looks fine - maybe it got corrupted during the first download? Will try re-downloading. Meanwhile, I'll try running the full list and seeing what else fails :/

####2017-09-16

Not everything failed! So upping the RAM, restraining computers to 64 bit, and copying the gluster files over helped something. 271 files worked, 873 did not. My hypothesis is that the ones that failed are from GEODES006, which has missing sequences according to the gt errors. And it's true! So I'll move the successful files over, remake the list of files that failed, and checksum my hard drive copy. If that matches what's on gluster, I'll re-download the fasta file and try again. 

Re run the phylodist steps - remember GEODES006 had that funky Curvibacter line. Was it the truncated locaton?
Grep says it looks that way! There are lots of curvibacter lines and the sp. field is repeated twice to make 8 fields total. Move on to splitting, run phylodist, and concatenate.
...
I want to redo all of the GEODES006 gff parts.

####2017-09-25

Not much happened since my last entry, because clustering large files is slow. After running the gff building script on GEODES006 (with no issues!), I ran the cd-hit script. It ran for 3 days before getting kicked off. I wasn't sure if this was due to the clustering or the gff building, so I took out the gff building from the script and just ran the cd-hit part. That also ran for 3 days. I tried again, increasing its RAM, which did not work. Finally, I read in the cd-hit manual that it can be run on multiple threads, and I managed to find a couple execute nodes with 16 threads and 24 GB RAM. There's only 4 in the system, but I only need one. This ran in about 36 hours - so plenty of time to spare! Which is good, because I will need to add the SAGs in eventually, plus maybe some other published reference genomes.

This was so successful that I ran the bbmap index step right away (this was yesterday) and it finished in a couple hours! I think the bulk of this time was actually unzipping the database fna file. This also means that I can probably turn off the usemodulo flag, which increases speed at the cost of accuracy. But for this run, I'll keep it because I need results fast before lab meeting next week.

I can start the mapping right away, which shouldn't take very long. But after that, I'll need the gff file for the non-redundant database. I'm thinking of splitting that up too, since the grepping takes forever and a half (24+ hours on just the ref MAGs and SAGs). I'll use the metaG gff building script as a template.

Also of note, I zipped up the metagenome_assemblies folder in gluster and deleted the "filtered" RNA files to make space for mapping results. I'm currently at 160 files and 150 GB. I'll need at least another 110 for the mapping output.

split_fastaheaders.sh
```{bash, eval = F}
#!/bin/bash
mkdir split_fastaheaders
split -l 5000 -a 4 -d /mnt/gluster/amlinz/fasta_headers.txt split_fastaheaders/fasta_headers

#5000 should aim for about 2300 jobs
# Make a list of files to run - only doing a couple to test
ls split_fastaheaders > split_fastaheaders.txt
```
...
output = 07nrdb_gff__$(Cluster).out
#
# Specify that HTCondor should transfer files to and from the
#  computer where each job runs.
should_transfer_files = YES
when_to_transfer_output = ON_EXIT
transfer_input_files = split_fastaheaders/$(filepart)
...
#
# run from list
queue filepart from split_fastaheaders.txt
...
paste genome.txt clusters.txt type.txt start.txt endpoint.txt info1.txt info2.txt info3.txt tags.txt > $1_nrdb.gff
...
In retrospect, I also need to remove whitespace from fasta headers before indexing, or featureCounts will truncate. This is specifically an issue in product names. I'll do this manually for now but keep that in mind when naming things when adding SAGs to the database. Using tr -d ' '

Run a few test files through

####2017-09-26

There's one line causing an issue in the gff file. It looks like no cluster was found for a single gene, so that row is short a column. I tried fixing with awk or sed (below) with no success. Removed this one gene (it was a Sox and there are 300 more like it) for now but will need to address the underlying issue of the missing cluster when I add the SAGs to the database.

```{bash, eval = F}
awk '{gsub(/CDS/,"Unknown\tCDS",$2);print}' /mnt/gluster/amlinz/unprocessed-nrdb.gff > /mnt/gluster/amlinz/semiprocessed-nrdb.gff

awk 'FNR == 9311290 {print $2}' /mnt/gluster/amlinz/semiprocessed-nrdb.gff
awk 'FNR == 9311290 {print}' /mnt/gluster/amlinz/semiprocessed-nrdb.gff
```

Restarted with that line deleted.

####2017-09-27

Started the mapping! One test file worked great so I started the rest. Only changes needed to the existing 09mapping scripts was paths.

####2017-09-28

Looks like some files are failing because the execute node cannot find samtools? That's odd that some would find it and others wouldn't. Will wait to see how many do this.

####2017-10-02

About 1/4 of the files failed, all for that same reason. Added a specification for 64 bit system (this solved a similar issue with genometools earlier this month) and started rerunning the files that failed.

####2017-10-03

1/4 of the files are still failing, although that leaves me with only 8. Still the same reason - can't find samtools executable. Tried changing the script to specify the direct path of samtools instead of adding it to PATH variable - this worked on 1 file but hard to tell as it seems to be random execute nodes causing the problem.

####2017-10-04

Nearly done - only 3 files left! The direct path seems to be working (or maybe I'm just lucky). 

####2017-10-05

The mapping is all done! Modifying the old featurecounts scripts now and will run on one to test. The only change I'm planning to make is to reference files directly in gluster instead of copying the massive things to an execute node, and saving output to my home folder instead of gluster since each individual file shouldn't be too large (but will check that on the test file).

Unzip the gff file and make a list of the samples (no file extensions)

```{bash, eval = F}
gzip -d /mnt/gluster/amlinz/nonredundant_database.gff.gz

for file in /mnt/gluster/amlinz/GEODES_mapping_results/*; do sample=$(basename $file |cut -d'.' -f1); echo $sample;done > bamfiles.txt

# make it a test file for now
head -1 bamfiles.txt > temp.txt && mv temp.txt bamfiles.txt
...
#Count reads
./subread-1.5.2-source/bin/featureCounts -t CDS -g locus_tag --minOverlap 1 --fracOverlap 1 -M --fraction -a /mnt/gluster/amlinz/nonredundant_database.gff -o $1.CDS.txt /mnt/gluster/amlinz/GEODES_mapping_results/$1.mapped.bam
...
transfer_output_files = $(samplename).CDS.txt
#
# Tell HTCondor what amount of compute resources
#  each job will need on the computer where it runs.
Requirements = (Target.HasGluster == true)
...
Testing on GEODES001

Error #1: I told it to read locus_tags instead of IDS. changed and start over.
Error #2: SAM_pairer_get_next_read_BIN: Assertion `l_name < 256' failed.
I think this means that it doesn't like the long gene names? I can't really shorten them... can't open the executable in nano to change... ugh.

Having a hard time finding the source code for the Linux version but in the Rsubreads package, there is indeed a setting of "CHROMOSOME_NAME_LENGTH = 256".

<<<<<<< HEAD
Actually, looks like my genome names are more around 100-150 characters long, and the error seems to be specific for the chromosome names. But my chromosome name is always "NR_gene_database", or ">Cluster 1". I see 2 potential issues - one is the space in the cluster name. The other is there's a ### line in between all of my gff entries and I don't know why. I'm going to make a very small gff file that I can run tests on.
=======
####2017-10-09

I asked for help and may have solved 2 problems with one stone. Josh, Elizabeth, and Sarah helped me write a script for a Python dictionary, which will allow me to use the shorter gene IDS/locus tags with an accompanying key file, and this whole process is at least 4x faster than grep. I think the short names should solve the featurecounts error because I've gotten to run on parts of the database before. And the shorter names will allow me to better parse the cluster file.

To make sure this works, I'm going to map metatranscriptomic reads to the ref MAGs SAGs because Trina wanted this anyway and make sure I can count and use the dictionary properly. After that, I'll try clustering just the ref MAGs SAGS to see if I can preferentially choose gene names. But first, I need to add a package to my Python tarball. This package processes GFF files!
...
#python bcbio_nextgen_install.py $(pwd)/python/local/share/bcbio --tooldir=$(pwd)/python/local --nodata -->
...
Tested Josh's script and it is working! Now to try a ref MAG gff file.
...
CHTC was down for reboots because of a security issue, but we're good now. Meanwhile, I made few more changes to the python script for parsing MAGs and SAGs. Here's the new python script:
...
My work with Josh can be found here: https://github.com/alexlinz/geodes_mwe

I modified Josh's version to change the ID to the locus tag, to make a column of genome in the output file, and to take an argument that is the name of the genome to test. Now to put it in with the rest of the processing script:
...
should_transfer_files = YES
when_to_transfer_output = ON_EXIT
transfer_input_files = /home/amlinz/zipped/genometools.tar.gz,/home/amlinz/ref_MAGs_SAGs/fastas/$(samplename).fna,/home/amlinz/ref_MAGs_SAGs/gffs/$(samplename).gff,/home/amlinz/ref_MAGs_SAGs/Readme.csv,scripts/ref_MAGs_SAGs_parsing.py,zipped/python.tar.gz
transfer_output_files = CDS$(samplename).fna,sorted$(samplename).gff,$(samplename)table.txt
#
# Tell HTCondor what amount of compute resources
#  each job will need on the computer where it runs.
# Requirements = (Target.HasGluster == true)
...
Make a test list with scripts/refMAGs_SAGS_list.sh

Had to change _ to . because variables. Started the full run and some of the things didn't work. Was super fast, though! No errors but occasional empty output. Will test these in interactive mode. Or is it the operating system requirements again? I'll test that while I go eat lunch.

Nope, didn't work. Closer inspection of one of the failed files shows it was fixed by removing CRISPR regions, though. Adding that to the main script. I also need to test the internal standard.

That fixed them! It looks like it doesn't even make sense to run the script on the internal standard, since it is the CDS region and it does not need to be in my key table. So I'll just use my premade files for those.

Now I need to concatenate the fasta files, merge the gff files, and use that as my database.

```{bash, eval = F}
# Easy enough to concat the fna files from my submit node

cat CDS.*.fna ref_MAGs_SAGs/fastas/pFN18A_DNA_transcript.fna > /mnt/gluster/amlinz/ref_MAGs_SAGs_database.fna

# Move the gff files into a folder for transport
mkdir ref_MAGs_SAGs_processed_gffs
mv sorted*.gff ref_MAGs_SAGs_processed_gffs/
mv ref_MAGs_SAGs/gffs/pFN18A_DNA_transcript.gff ref_MAGs_SAGs_processed_gffs/

```

In an interactive session (since I'm not going to write a whole script for one job):
```{bash, eval = F}
tar xvzf genometools.tar.gz
export PATH=$(pwd)/genometools/bin:$PATH

gt merge -retainids -tidy -o ref_MAGs_SAGs_database.gff *gff

mv ref_MAGs_SAGs_database.gff /mnt/gluster/amlinz/
```

Change build index scripts to process this database. I've renamed them to build_index2.
That worked 1st try! Now change name of the database in the mapping scripts and offit goes.


1st trial couldn't find the output file - test in interactive mode
...
I capitolized an S where I shouldn't have. Oops. It's 7PM so let's go for broke, submit the whole thing, and see if it's done by morning.

#2017-10-13

Not quite done yet (17/109 left) but they're all working! Meanwhile, I'll test out the featurecounts script. Hopefully it works now that I've got shorter gene names.
...
#Count reads
./subread-1.5.2-source/bin/featureCounts -t CDS -g ID --minOverlap 1 --fracOverlap 1 -M --fraction -a /mnt/gluster/amlinz/ref_MAGs_SAGs_database.gff -o $a.MAGsSAGs.CDS.txt /mnt/gluster/amlinz/GEODES_mapping_results/$a.MAGsSAGs.mapped.bam
...
There no ID feature in the gff file? That's weird. Redo the gff merge with better toggling of genometools options

```{bash, eval = F}
tar xvzf genometools.tar.gz
export PATH=$(pwd)/genometools/bin:$PATH

gt merge -retainids yes -tidy yes -o ref_MAGs_SAGs_database.gff *gff
...
Well featurecounts still isn't working. If I have to guess, the problem is that I did not readjust the start and end positions of the genes to 1 - length of gene. This wasn't an issue the 1st time around because I used whole contigs, and it wasn't an issue the 2nd time around because I reset the lengths after clustering. So this won't be an issue in the full run. For now, I'll move on to working on the full run. 1st up, test the gff parsing python script on the metagenome assemblies and the ref algae. Then grab a new SAG.
...
#Remove CRISPRs
grep -v "CRISPR" $a.gff > temp.gff && mv temp.gff $a.gff
grep -v "#!" $a.gff > temp.gff && mv temp.gff $a.gff
#Setup and run python script
chmod +x ref_MAGs_SAGs.py
python ref_MAGs_SAGs.py $a
...
Modify the python script to read the algae phylogeny file
...
I had another thought as I battled the sniffles yesterday. Why do I need the intermediate gff files? All I need is something good enough to extract CDS regions. Then I make the gff file that featurecounts will read from the clustering ouptut. All I need pre-clustering is to extract the info from the gff file into table form.

My tasks at hand:
- remove the gff building from the refMAGs SAGs script
- finish the algae script
- write a script for the metagenome assemblies
- download some of the new SAGs and write scripts for that
- cluster and write a script to make a gff file from the output
- see if featurecounts works

I just had a brilliant idea to fix the featurecounts issue in the MAGs SAGs run. Map to the whole genomes instead of just CDS regions. That way the numbers still match up, and the database size is still small enough that it doesn't matter if I'm mapping to extra. I still only need to count CDS regions.
...
cat ref_MAGs_SAGs/fastas/*pFN18A_DNA_transcript*.fna > /mnt/gluster/amlinz/ref_MAGs_SAGs_database.fna

#run 08buildindex2
#run 09mapping

for file in /mnt/gluster/amlinz/GEODES_mapping_results/*; do sample=$(basename $file |cut -d'.' -f1); echo $sample;done > bamfiles.txt
awk -F';' {'print $1'} /mnt/gluster/amlinz/ref_MAGs_SAGs_database.gff > temp.gff
mv temp.gff /mnt/gluster/amlinz/ref_MAGs_SAGs_database.gff
#run 10featurecounts
...
#Count reads
./subread-1.5.2-source/bin/featureCounts -t CDS -g ID --minOverlap 1 --fracOverlap 1 -M --fraction -a gffs/2582580615.gff -o test.MAGsSAGs.CDS.txt GEODES121-nonrRNA.MAGsSAGs.mapped.bam
...
No problem here - try the mapping?
```{bash, eval = F}
...
I'm an idiot. I told it the wrong index name.


####2017-10-23

Back to working on the algae gff parsing
...
a="GCF_000149405.2_ASM14940v2_genomic"
gzip -d fastas/$a.fna.gz
mv fastas/$a.fna .
gzip -d gffs/$a.gff.gz
mv gffs/$a.gff .
...
for record in GFF.parse(inFile, target_lines = 100):
    print record
```
...
Upon inspecting the refMAGs SAGS run files, it looks like the gff files are wrong. There's no IDs and there's no standard. Take 2 to fix this:

```{bash, eval = F}
sed '2q;d' pFN18A_DNA_transcript.gff > line2.gff
cat sorted.2619619040.gff line2.gff > temp && mv temp sorted.2619619040.gff
gt gff3 -sort yes -tidy -force -retainids -o ref_MAGs_SAGs_database.gff *gff
```

I get warnings about ids not being unique but I think that's ok for now. Try featurecouns with this file - at the very least, it should pick up the standard.

Taht worked! re run the bamfiles.txt list and count em all.
...
Everything worked great post-mapping last week, and I was able to get a table of read counts (normalized to transcripts/L) by sample, and aggregate that by genome. Now that I have that working, I have several challenges left to get the full database up and running. One, I need to make the gff renaming and key file generating script generalizable to all my types of data (I already started this with the algae, and the new SAGs look the same as the old SAGs, so really the assemblies are the problem). Two, I need to make a new gff file from the clustering output, preferably without using grep. Three, I need some way to QC the clusters - how many genes/cluster? Are they all the same gene annotation/phylogeny? Can I preferentially pick a ref seq from the MAGs or SAGs since they are more likely better classified than the assemblies? And four, I need to do some QC on my database - how much redundancy is there? How well represented are the ref MAGs and SAGs in the metagenome assemblies?

Let's start easy and work up. 1st up, modify my ref MAGs SAGs script to work for the full database. The changes I want are:
- Use the gff file to extract CDS regions, but then don't save the gff file
- Save the CDS fasta but not the other one
- Add one of the new SAGs in and see if the script still works
...
should_transfer_files = YES
when_to_transfer_output = ON_EXIT
transfer_input_files = /home/amlinz/zipped/genometools.tar.gz,/home/amlinz/ref_MAGs_SAGs/gffs/$(samplename).gff,/home/amlinz/ref_MAGs_SAGs/Readme.csv,scripts/ref_MAGs_SAGs-parsing.py,zipped/python.tar.gz, ref_MAGs_SAGs/fastas/$(samplename).fna
...
No changes needed to python script.
...
Lastly, add table.txt file moving to the postprocessing script.
move_CDS_output.sh:
```{bash, eval = F}
#!/bin/bash
...
mv /home/amlinz/CDS*.fna /home/amlinz/CDS_fastas
cat /home/amlinz/CDS_fastas/* > /mnt/gluster/amlinz/CDS_regions.fna
cat /home/amlinz/*.table.txt > /mnt/gluster/amlinz/CDS_regions_genekey.txt
gzip /mnt/gluster/amlinz/CDS_regions.fna

```


For testing purposes, modify the ref MAGs SAGs list to include one time series MAG or SAG and one new SAG. 2754412703

Success! Looks like I can just dump the new SAGs in with the old ones and append their phylogeny info to the Readme.csv file.

Next up, run an interactive session to write the python script for the metagenome assemblies. Will work on that after lunch, though.


####2017-10-31

Happy Halloween! Got caught up in meetings/emails/ordering yesterday afternoon so coming back to the metagenome assemblies now. I'll grab one of the assemblies and the ref MAGs SAGs script and work on modifying it in an interactive session.
...
transfer_input_files = zipped/python.tar.gz,zipped/genometools.tar.gz,refseq_algae/,scripts/ref_MAGs_SAGs.py
#transfer_input_files = zipped/genometools.tar.gz,ref_MAGs_SAGs_processed_gffs/
...
In the interactive sesssion:
```{bash, eval = F}
# Set test variable
a="GEODES006"

# Copy fasta, gff, phylodist, and product_names from gluster
cp /mnt/gluster/amlinz/metagenome_assemblies/fastas/$a.assembled.fna.gz .
cp /mnt/gluster/amlinz/metagenome_assemblies/gff/$a.assembled.gff .
cp /mnt/gluster/amlinz/metagenome_assemblies/phylogeny/$a.assembled.phylodist .
cp /mnt/gluster/amlinz/metagenome_assemblies/product_names/$a.assembled.product_names .

# Unzip fasta file
gzip -d $a.assembled.fna.gz

# What needs to change in the python script?
# - Keep renaming in the gff but that's all it's good for
# - Use phylodist for phylogeny information, but this is now by contig rather than by genome
# - Look up annotations in product_names instead of gff
...
mv ref_MAGs_SAGs_parsing.py metaG_parsing.py
...
genome = sys.argv[1]
#genome = 'GEODES006'
taxonFile = genome + '.assembled.phylodist'
...
readme = pandas.read_csv(taxonFile, sep='\t')
readme.columns = ['Contig', 'Thing1', 'Thing2', 'Conf', 'TaxString']
...
for record in GFF.parse(inFile, limit_info=limit_info):
    for seq in record.features:
        ...
        taxonomy = taxonDict.get(seq.id[0:17])
        product = productDict.get(seq.id)
        outFile2.write(seq.qualifiers['ID'][0]+'\t'+genome+'\t'+str(taxonomy)+'\t'+str(product)+'\n')
    GFF.write([record], outFile1)
...
So it doesn't work when I run the full script, but does work when I do large subsets (and fast). Is there a weird line in GEODES006? Tomorrow I'll try removing CRISPR lines or splitting up into chunks to see which lines don't work.


####2017-11-01

Let's try interactive mode again, but with GEODES117 this time.
Nope. Try removing CRISPR sequences.
Nope. Try running the full gff fixing thing I did originally.
...
ITS WORKING. Slowly but surely. it's done ~800,000 in the last hour so may need to split? Maybe not. Total is 4-5 million so 6hrs or so isn't a bad time.

Actually it got stuck at 850000. Maybe there is a bum line in there?

####2017-11-2

Try again, trying to find the problem line. I'll run gt tidy 1st and split the gff file into smaller sections.

Split the gff file by line numbers:
```{bash, eval = F}
mkdir gff_test
split -500000 /mnt/gluster/amlinz/metagenome_assemblies/gff/GEODES006.assembled.gff
cd ..
# make list of files to run
ls gff_test > gff_test.txt
```
...
cp /mnt/gluster/amlinz/metagenome_assemblies/phylogeny/GEODES006.assembled.phylodist .
...
echo '##gff-version 3' | cat - f1_GEODES006.assembled.gff > temp && mv temp f1_GEODES006.assembled.gff
gt gff3 -sort yes -tidy -retainids -force -o GEODES006.assembled.gff f1_GEODES006.assembled.gff

python metaG_parsin.py "GEODES006"
...
transfer_output_files = $(samplename).parsed.gff,$(samplename).table.txt
#
# Tell HTCondor what amount of compute resources
#  each job will need on the computer where it runs.
# Requirements = (Target.HasGluster == true)
...
Woohoo, that worked! Nothing failed, even. Not sure why. One quick test to see if anything come through that was classified

```{bash, eval = F}
for file in x*table.txt; do awk {'print $3'} $file | grep -v "None" | wc -l; done

```

Nothing. That's odd. Is there anything classified in the phylodist file? Yes, which means I probably wrote that part of the python script wrong. Yep, looks like I'm short 1 character of the contig name. Darn python.

I'll test this while I'm adding on the CDS fasta bit to script.
...
rm -rf python
```
Change the submit script to keep the fasta but not the gff. Test on one file.

Finished and output looks good, but still no classifications. Further testing reveals that I was giving it JGI's phylodist file instead of my contig classification file *headkeyboard*. Let's try this again...
...
IT WORKED!!!!! My next goal is to modify the algae and ref genomes to only save the CDS regions and not the gff file. In retrospect, I'm going to modify the python scripts to append genome to the end of the ID so that if anything gets truncated in the clustering key file, its that. Then I'll run everything I've got through the pipeline, cluster and start working on the next python challenge.

Remake the ref MAGs SAGs list and submit jobs:
```{bash, eval = F}
for file in ref_MAGs_SAGs/fastas/*;do name=$(basename $file | cut -d'.' -f1); echo $name; done > refMAGs_SAGs_list.txt
```
...
Now I need to remake the gff_test scripts to span multiple metagenomes. I think if I split all of them, keep the metagenome name in the split file name, and then trim that into a variable, it should work.
...
ls metaG_gffs > metaG_gffs.txt
```
So far so good. Modify the executable to take a variable
...
rm -rf python
```
And a new submit file to match:
...
should_transfer_files = YES
when_to_transfer_output = ON_EXIT
transfer_input_files = zipped/python.tar.gz,zipped/genometools.tar.gz,scripts/metaG_parsing.py,metaG_gffs/$(samplename)
transfer_output_files = CDS.$(samplename).fna,$(samplename).table.txt
#
# Tell HTCondor what amount of compute resources
#  each job will need on the computer where it runs.
...
#
# Tell HTCondor to run every fastq file in the provided list:
queue samplename from 05metaG_gffs.txt

```

Run on 2 test files first

That works great! However, I just realized I never actually changed the algae executable to run the python script. This one is fast enough that it's not a problem, but I want all the formats and sample names to match.
...
transfer_output_files = CDS_$(samplename).fna,$(samplename).table.txt
#
# Tell HTCondor what amount of compute resources
#  each job will need on the computer where it runs.
# Requirements = (Target.HasGluster == true)
...
I'm resorting to awk
```{bash, eval = F}
awk -F'\t' -v OFS='\t' '{print $9}' $a.parsed.gff > tags.txt
awk -F";" '{for(i=1;i<=NF;i++){if ($i ~ /ID=*/){print $i}}}' tags.txt > IDs.txt


=======
####2017-11-07

After bashing my head against the algae gff files, I asked Josh for help. He says the problem is because it's eukaryotes and they can have multiple CDS regions in the same gene regions. The gff writing inferred parent lines seems to be an inherent property of GFF.write that Josh couldn't fix, so he recommends removing those lines via sed. I'll also need to remove those lines in my table - should be straightforward as I think they are called "rna" instead of "cds" - and possibly remove extra info in the parsed gff file so it doesn't go look for parent lines. Booting into interactive mode to rewrite.
...
#Remove all the duplicate gff-version lines
sed -i '/##gff-version 3/d' $1.parsed.gff
# Move sequence region lines to top of file
grep "##sequence-region" $1.parsed.gff > sequence_regions.gff
sed -i '/##sequence-region/d' $1.parsed.gff
sed -i '/inferred_parent/d' $1.parsed.gff
sed -i '/Parent=*;/' GCF_000149405.2_ASM14940v2_genomic.parsed.gff > test.gff

# Put it all back together and clean up
echo '##gff-version 3' | cat - sequence_regions.gff $1.parsed.gff > $1.fixed.gff
gt gff3 -sort yes -tidy -retainids -o sorted.$1.gff $1.fixed.gff #clean up the the gff sorter
...
To get genometools to run on the file with no inferred problems, I had to delete the Parent= qualifier. But then I realized that spliced genes means there are multiple coding regions with the same names, presumably because they end up being translated together. But that doesn't really help me. I'm going to try using the python script to match products with genes AFTER I make a gff file using my previous bash script.
...
Back in my submit folder, I modified the algae executable and python script to match what I did in the interactive session above. The submit file doesn't need to change. Testing on one algal genome to start.

Woohoo! Success! Now to put all the various scripts together. I'm going to take those commands plus the split metaG gff script and put them together into a single script.
...
```
Submit the jobs. They can all be started at the same time.
```{bash, eval = F}
condor_submit submits/03refMAGs_SAGs.sub
condor_submit submits/04algae.sub
condor_submit submits/05metaG_gffs.sub

```

Modify post processin script to include the standard
```{bash, eval = F}
#!/bin/bash

mv /home/amlinz/CDS*.fna /home/amlinz/CDS_fastas
cat /home/amlinz/CDS_fastas/* /home/amlinz/ref_MAGs_SAGs/fastas/pFN18A_DNA_transcript.fna > /mnt/gluster/amlinz/CDS_regions.fna
echo "pFN18A_DNA_transcript internal standard internal standard internal standard" > std.table.txt
cat /home/amlinz/*.table.txt > /mnt/gluster/amlinz/CDS_regions_genekey.txt
gzip /mnt/gluster/amlinz/CDS_regions.fna

```

All good! Now the clustering. upping the RAM and hard drive space because my zipped .fna file isalready 5GB. And I'm not going to keep/make the fasta headers file since I'll be writing a more sophisticated python script for that.
...
In the executable, change the mapping ID level and remove the fastaheaders part
06cd-hit.sh
```{bash, eval = F}
...
./cd-hit-v4.6.8-2017-0621/cd-hit-est -i CDS_regions.fna -o nonredundant_database.fna -c 0.80 -M 24000 -T 16
...
```
<<<<<<< HEAD
>>>>>>> afc180f7034b90b6b6d311a3d713fb3c622f7ec4
=======
####2017-11-26

I've been quiet lately because gluster is down. It died while I was in the middle of uploading the new Sparkling Lake metagenome assemblies. I managed to run the 02phylodist scripts on 3/4 assemblies, but the last one kept failing for, as far as I can tell, bad hard drive reasons. CHTC has been trying to fix it for nearly 2 weeks now, and told me right before Thanksgiving that I should find workarounds and not wait for them. I'll do that where I can, although the clustering is not happening without Gluster. Today I modified my phylodist script to transfer the GEODES168 phylodist file from my home folder, even though it's technically a little too large for that (180MB, general rule is 100MB).


####2017-12-01

After discussion with CHTC, I have access to SQUID, which is sort of an in between file transfer system. To recap:
- My submit node can only transfer files up to 100MB
- SQUID can do 100MB-1GB and is faster than Gluster. Intend for files referenced by multiple jobs
- Gluster is needed for anything over 1GB. Intended for files unique to a job.

Unfortunately, nearly everything I need to work with has at least one file over 1GB. I poked some files in my Gluster folder and they seem to be working ok (although I will need to reupload all my metatranscriptomes... ugh). My goal for today is to continue working towards my non-redundant database by rewriting 05metagenome_assemblies to reference SQUID where possible and copy from Gluster to an execute node instead of reading from Gluster directly. I'll then check with CHTC to make sure I'm doing it right.
...
metaG=$(echo $1 | cut -c1-9)
tar xvzf $metaG.datafiles.tar.gz
tar xvzf genometools.tar.gz
export PATH=$(pwd)/genometools/bin:$PATH
...
Realized I need the version with the python script, 05metaG_gffs. The squid stuff worked and should transfer over neatly, though.
...
should_transfer_files = YES
when_to_transfer_output = ON_EXIT
transfer_input_files = zipped/python.tar.gz,zipped/genometools.tar.gz,scripts/metaG_parsing.py,metaG_gffs/$(samplename),http://proxy.chtc.wisc.edu/SQUID/amlinz/GEODES165.datafiles2.tar.gz
transfer_output_files = CDS.$(samplename).fna,$(samplename).table.txt
#
# Tell HTCondor what amount of compute resources
#  each job will need on the computer where it runs.
...
#
# Tell HTCondor to run every fastq file in the provided list:
queue samplename from metaG_gffs.txt

```

Test worked! I made datafile2.tar.gz tarballs for all of the metagenomes. Unfortunately, I think I'm going to need a seperate submit file for each metagenome, which is a bit of a pain. I'll modify the splitting script to split all, but then make separate lists for each 
```{bash, eval = F}
...
split -500000 /mnt/gluster/amlinz/metagenome_assemblies/gff/GEODES117.assembled.gff GEODES117.
split -500000 /mnt/gluster/amlinz/metagenome_assemblies/gff/GEODES118.assembled.gff GEODES118.
split -500000 /mnt/gluster/amlinz/metagenome_assemblies/gff/GEODES165.assembled.gff GEODES167.
...
cd ..
# make list of files to run
ls metaG_gffs/GEODES005* > 005metaG_gffs.txt
ls metaG_gffs/GEODES006* > 006metaG_gffs.txt
ls metaG_gffs/GEODES057* > 057metaG_gffs.txt
ls metaG_gffs/GEODES058* > 058metaG_gffs.txt
ls metaG_gffs/GEODES117* > 117metaG_gffs.txt
ls metaG_gffs/GEODES118* > 118metaG_gffs.txt
ls metaG_gffs/GEODES165* > 165metaG_gffs.txt
ls metaG_gffs/GEODES166* > 166metaG_gffs.txt
ls metaG_gffs/GEODES167* > 167metaG_gffs.txt
ls metaG_gffs/GEODES168* > 168metaG_gffs.txt

```

It's working! I started ALL of my CDS regions :) Modifying the post CDS processing script to not save everything to gluster once this is done.
...
#make dummy table of  "pFN18A_DNA_transcript\tinternal standard\tinternal standard\tinternal standard" 1st. Named pFN18A.table.txt

mv /home/amlinz/CDS*.fna /home/amlinz/CDS_fastas
cat /home/amlinz/CDS_fastas/* /home/amlinz/ref_MAGs_SAGs/fastas/pFN18A_DNA_transcript.fna > CDS_regions.fna
cat /home/amlinz/*.table.txt > CDS_regions_genekey.txt
...
The resulting files are 3.2GB for the fasta (zipped) and 2.5 for the genekey. They seem to be doing just find in Gluster so I will check the submit file to make sure it has enough space and start that overnight.

####2017-12-6

Clustering finished last night! Working on a way to inform how i choose rep seqs for each cluster of nonredundant CDS regions. See clusters.Rmd for details (this file is opening kind of slowly...)

```{bash, eval = F}
maxsize=800
touch endpoint.txt
cat split_fastaheaders/fastaheaders0036 | while read line; do var=$(echo $line | cut -c1-19); grep -B $maxsize $var /mnt/gluster/amlinz/nonredundant_database.fna.clstr > splitfile.clstr; cluster=`grep "Cluster" splitfile.clstr | tail -1`; echo $cluster; length=`tail -1 splitfile.clstr | awk '{print $2}' | sed 's/[^0-9]*//g'`; echo $length >> endpoint.txt;done > clusters.txt

>ME1300DRAFT_00843_2582580525

```

####2017-12-07

After running all 9800 jobs, I realized I was unnecessarily truncating samplenames and not unzipping the .clster file. Started a few testers last night, which worked great, and started the rest this morning. I wrote to Josh and Sarah about CD-HIT's rep seq choosing, and they agree that the longest gene is probably the most informative rep seq, and there is no need to write a script to rename genes. So I will continue on my merry way with the mapping, once the gff file is made.

But first, I need to update my workflow before I get too far and forget how I did this. 

>>>>>>> c386a62a0246dc46d7cc595bd6f0f02d3b98a54d
