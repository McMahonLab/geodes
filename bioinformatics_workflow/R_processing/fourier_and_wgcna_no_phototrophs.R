...
# I'm screening photosynthetic genes out of this analysis.
# Will need to do some key word searching for photosynthesis related terms

# test1 <- grep("photo", mendota_key$Product, invert = T)
# test2 <- grep("chlorophyll", mendota_key$Product, invert = T)
# genes2keep <- mendota_key$Gene[intersect(test1, test2)]
# rows2keep <- match(genes2keep, rownames(mnorm))
# mnorm <- mnorm[rows2keep,]
# colnames(mnorm) <- gsub(".nonrRNA", "", colnames(mnorm))

# test1 <- grep("photo", spark_key$Product, invert = T)
# test2 <- grep("chlorophyll", spark_key$Product, invert = T)
# genes2keep <- spark_key$Gene[intersect(test1, test2)]
# rows2keep <- match(genes2keep, rownames(snorm))
# snorm <- snorm[rows2keep,]
# colnames(snorm) <- gsub(".nonrRNA", "", colnames(snorm))

test1 <- grep("photo", trout_key$Product, invert = T)
test2 <- grep("chlorophyll", trout_key$Product, invert = T)
genes2keep <- trout_key$Gene[intersect(test1, test2)]
rows2keep <- match(genes2keep, rownames(tnorm))
tnorm <- tnorm[rows2keep,]
...
#spark_net <- blockwiseModules(sig.spark, maxBlockSize = 1000, power = 18, loadTOM = F, saveTOMs = F, networkType = "signed", minModuleSize = 30, numericLabels = T, nThreads = 8, verbose = 3)
...
# Save output
# write.csv(sig.mendota.key, "D:/geodes_data_tables/WGCNA_mendota_results_nophoto_2018-03-30.csv", row.names = F)
# write.csv(mendota_net$MEs, "D:/geodes_data_tables/WGCNA_mendota_eigenvectors_nophoto_2018-03-30.csv", row.names = T)

# write.csv(sig.spark.key, "D:/geodes_data_tables/WGCNA_sparkling_results_nophoto_2018-03-30.csv", row.names = F)
# write.csv(spark_net$MEs, "D:/geodes_data_tables/WGCNA_sparkling_eigenvectors_nophoto_2018-03-30.csv", row.names = T)

write.csv(sig.trout.key, "D:/geodes_data_tables/WGCNA_trout_results_nophoto_2018-03-30.csv", row.names = F)
write.csv(trout_net$MEs, "D:/geodes_data_tables/WGCNA_trout_eigenvectors_nophoto_2018-03-30.csv", row.names = T)
