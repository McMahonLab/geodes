...
keep <- err_mnorm$Genes[which(err_mnorm$value < 40)]

new_abun_mnorm <- new_abun_mnorm[, which(colnames(new_abun_mnorm) %in% keep)]

# Keep only non-hypothetical proteins

keep <- mendota_key$Gene[grep("hypothetical|unknown|unclassified", mendota_key$Product, invert = T)]
new_abun_mnorm <- new_abun_mnorm[, which(colnames(new_abun_mnorm) %in% keep)]

# Keep only vaguely classified genes
keep <- mendota_key$Gene[grep("unclassified|Unclassified|Blank|NO CLASSIFICATION", mendota_key$Taxonomy, invert = T)]
new_abun_mnorm <- new_abun_mnorm[, which(colnames(new_abun_mnorm) %in% keep)]
...
# Make the modules
mendota_net <- blockwiseModules(new_abun_mnorm, maxBlockSize = 1000, power = 8, loadTOM = F, saveTOMs = F, networkType = "signed", minModuleSize = 30, numericLabels = T, verbose = 3, threads = 8)
...
# Save output
write.csv(sig.mendota.key, "D:/geodes_data_tables/WGCNA_mendota_results_nondiel_2018-04-04.csv", row.names = F)
write.csv(mendota_net$MEs, "D:/geodes_data_tables/WGCNA_mendota_eigenvectors_nondiel_2018-04-04.csv", row.names = T)
