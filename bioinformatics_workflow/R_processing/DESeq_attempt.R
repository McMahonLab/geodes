...
ggplot(data = plot.lake_comparison[which(plot.lake_comparison$Category != "Photosystem I" & plot.lake_comparison$Category != "RuBisCO" & plot.lake_comparison$Category != "Photosystem II"), ], aes(x = Category, y = value, fill = Lake)) + geom_bar(stat = "identity", position = "dodge") + scale_fill_manual(values = c("#5ab4ac", "#91bfdb", "#d8b365")) + labs(x = NULL, y = "Mean Transcripts/L") + coord_flip()
