...
```{r, fig1, eval = T, echo = F, fig.height = 18, fig.width = 18, warning = F, message = F}
...
p1 <- ggplot(mendota_phyla[which(mendota_phyla$Taxonomy != "Chloroflexi"), ], aes(x = metaG, y = value, color = type)) + geom_point(size = 2.5) + geom_label_repel(aes(label = Taxonomy, color = type), force = 10, size = 6) + scale_color_manual(values = c("green", "darkgrey", "royalblue", "black", "goldenrod")) + labs(x = "Mean proportion of metagenomic reads assigned", y = "Mean transcripts/L assigned", title = "Lake Mendota") + theme(legend.position = "none")
...
p2 <- ggplot(spark_phyla, aes(x = metaG, y = value, color = type)) + geom_point(size = 2.5) + geom_label_repel(aes(label = Taxonomy, color = type), force = 10, size = 6) + scale_color_manual(values = c("green", "red", "darkgrey", "royalblue", "black", "goldenrod")) + labs(x = "Mean proportion of metagenomic reads assigned", y = "Mean transcripts/L assigned", title = "Sparkling Lake") + theme(legend.position = "none")
...
p3 <- ggplot(trout_phyla, aes(x = metaG, y = value, color = type)) + geom_point(size = 2.5) + geom_label_repel(aes(label = Taxonomy, color = type), force = 10, size = 6) + scale_color_manual(values = c("green", "red", "darkgrey", "royalblue", "black", "goldenrod")) + labs(x = "Mean proportion of metagenomic reads assigned", y = "Mean transcripts/L assigned", title = "Trout Bog") + theme(legend.position = "none")
...
```{r, eval = T, warning = F, message = F, echo = F, fig.height = 5, fig.width = 15}
...
colorkey <- data.frame(phylum = c("Actinobacteria", "Alphaproteobacteria", "Armatimonadetes", "Bacteroidetes", "Betaproteobacteria", "Cyanobacteria", "Deltaproteobacteria", "Other", "Planctomycetes", "Unclassified", "Verrucomicrobia"), color = c("chocolate1",  "hotpink2", "turquoise3", "goldenrod1", "dodgerblue", "green3", "skyblue", "honeydew4", "lightcoral", "honeydew3", "lightslateblue"))

p1 <- ggplot(data = sig.res.key[which(sig.res.key$Category == "Photosynthesis" | sig.res.key$Category == "RuBisCO" | sig.res.key$Category == "Ribose transport" | sig.res.key$Category == "General sugar transport" | sig.res.key$Category == "Raffinose/stachyose/melibiose transport" | sig.res.key$Category == "Glucose/mannose transport" | sig.res.key$Category == "Rhamnose transport" | sig.res.key$Category == "Xylose transport" | sig.res.key$Category == "Fructose transport" | sig.res.key$Category == "Chitobiose transport" | sig.res.key$Category == "Lactose/arabinose transport" | sig.res.key$Category == "ROS" | sig.res.key$Category == "Nitrogenase"), ], aes(x = Condition, y = totals, fill = Condition)) + geom_bar(stat = "identity", position = "dodge") + labs(title = "Lake Mendota", x = NULL, y = "Transcripts/L") + scale_fill_manual(values = c("darkgoldenrod2", "royalblue4")) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_y_continuous(expand = c(0, 0)) + facet_wrap(~ Category, scales = "free_y")

# Plot taxonomy information as proportional bars
sig.res.key2 <- sig.res.key[grep("Photosynthesis|RuBisCO|Ribose transport|General sugar transport|Raffinose/stachyose/melibiose transport|Glucose/mannose transport|Rhamnose transport|Xylose transport|Fructose transport|Chitobiose transport|Lactose/arabinose transport|ROS|Nitrogenase", sig.res.key$Category), ]
...
**Figure 3. Differential expression in day vs. night in Lake Mendota.** Many genes were differentially expressed depending on light conditions in Lake Mendota. Genes related to photosynthesis and carbon fixation (via RuBisCO) were more expressed in the day, while genes related to sugar transport were more highly expressed at night, regardless of type of sugar transport encoded. Expressed photosynthesis genes were largely classified as Cyanobacteria or Bacteroidetes, while sugar transport genes were primarily classified as Actinobacteria or Bacteroidetes. Xylose transport, however, was primarily encoded by Cyanobacteria. 
...
# simple.sig.res.key <- simple.sig.res.key[grep("Photosynthesis|Polyamines|Rhodopsin|ROS|RuBisCO|Sugar degradation|Carboxylate", simple.sig.res.key$Category), ]

p1 <- ggplot(data = sig.res.key[which(sig.res.key$Category == "Photosynthesis" | sig.res.key$Category == "RuBisCO" | sig.res.key$Category == "Ribose transport" | sig.res.key$Category == "General sugar transport" | sig.res.key$Category == "Raffinose/stachyose/melibiose transport" | sig.res.key$Category == "Glucose/mannose transport" | sig.res.key$Category == "Rhamnose transport" | sig.res.key$Category == "Xylose transport" | sig.res.key$Category == "Fructose transport" | sig.res.key$Category == "Chitobiose transport" | sig.res.key$Category == "Lactose/arabinose transport" | sig.res.key$Category == "ROS"), ], aes(x = Condition, y = totals, fill = Condition)) + geom_bar(stat = "identity", position = "dodge") + labs(title = "Sparkling Lake", x = NULL, y = "Transcripts/L") + scale_fill_manual(values = c("darkgoldenrod2", "royalblue4")) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_y_continuous(expand = c(0, 0)) + facet_wrap(~ Category, scales = "free_y")

# Plot taxonomy information as proportional bars
sig.res.key2 <- sig.res.key[grep("Photosynthesis|RuBisCO|Ribose transport|General sugar transport|Raffinose/stachyose/melibiose transport|Glucose/mannose transport|Rhamnose transport|Xylose transport|Fructose transport|Chitobiose transport|Lactose/arabinose transport|ROS", sig.res.key$Category), ]
sig.res.key2$Phylum[which(is.na(sig.res.key2$Phylum) == T)] <- "Unclassified"
# Lump low abundance groups into "other"
sig.res.key2$Phylum[grep("Chloroflexi|Gammaproteobacteria|Gemmatimonadetes", sig.res.key2$Phylum)] <- "Other"
...
p2 <- ggplot(data = sig.res.key2, aes(x = Condition, y = totals, fill = Phylum)) + geom_bar(stat = "identity", position = "fill")  + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_y_continuous(expand = c(0, 0)) + labs(x = NULL, y = "Proportion of reads assigned", title = "Sparkling Lake") + facet_wrap(~ Category) + scale_fill_manual(values = as.character(colorkey$color[which((colorkey$phylum %in% unique(sig.res.key2$Phylum)) == T)]))
...
**Figure 4. Differential expression in day vs. night in Sparkling Lake.** Many genes were differentially expressed depending on light conditions in Sparkling Lake. Genes related to photosynthesis were more expressed in the day, while genes related to carbon fixation (via RuBisCO) and sugar transport were more highly expressed at night. Fewer types of expressed transporters for sugar were detected in Sparkling Lake compared to our other study sites, and expression of RuBisCO genes at night instead of during the day is contrary to the observed trends in Lake Mendota and Trout Bog. Expressed photosynthesis genes were largely classified as Cyanobacteria or Bacteroidetes, while sugar transport genes were primarily classified as Actinobacteria.
...
sig.res.key2$Phylum[which(is.na(sig.res.key2$Phylum) == T)] <- "Unclassified"
# Lump low abundance groups into "other"
sig.res.key2$Phylum[grep("Chloroflexi|Gammaproteobacteria|Gemmatimonadetes|Deinococcus-Thermus|Deltaproteobacteria", sig.res.key2$Phylum)] <- "Other"

# I don't want to look at taxonomy for genes and conditons with barely any expression
# Change these to zero based on composition of the day/night graph
sig.res.key2$totals[which(sig.res.key2$Category == "Photosynthesis" & sig.res.key2$Condition == "night" | sig.res.key2$Category == "RuBisCO" & sig.res.key2$Condition == "night")] <- 0


p2 <- ggplot(data = sig.res.key2, aes(x = Condition, y = totals, fill = Phylum)) + geom_bar(stat = "identity", position = "fill")  + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_y_continuous(expand = c(0, 0)) + labs(x = NULL, y = "Proportion of reads assigned", title = "Trout Bog") + facet_wrap(~ Category) + scale_fill_manual(values = as.character(colorkey$color[which((colorkey$phylum %in% unique(sig.res.key2$Phylum)) == T)]))
...
**Figure 5. Differential expression in day vs. night in Trout Bog.** Many genes were differentially expressed depending on light conditions in Trout Bog. Genes related to photosynthesis and carbon fixation (via RuBisCO) were more expressed in the day, while genes related tosugar transport were more highly expressed at night, regardless of the type of transporter. Expressed photosynthesis genes were classified as Cyanobacteria or unclassifiable, while sugar transport genes were primarily classified as Actinobacteria, Armatimonadetes, and Alphaproteobacteria, with a large proportion of reads remaining unclassified.
...
```{r, eval = F, echo = F, error = F, mesage = F}
# spark_within <- within_reps(snorm)
# spark_within <- apply(spark_within, 1, mean, na.rm = T)
# spark_across <- apply(snorm, 1, cv, na.rm = T)
# cv_stat <- c(rep("within replicates", length(spark_within)), rep("across samples", length(spark_across)))
...
# 
# # Save data
# write.csv(spark_cv_stats, file = paste(path2, "Desktop/geodes/intermediate_plotting_files/spark_cv_stats.csv", sep = ""), quote = F, row.names = F)

# Read data back in
spark_cv_stats <- read.csv(file = paste(path2, "Desktop/geodes/intermediate_plotting_files/spark_cv_stats.csv", sep = ""), header = T)


ggplot(spark_cv_stats, aes(x = stat, fill = type)) + geom_density(alpha = 0.5) + scale_fill_manual(values = c("#d8b365", "#5ab4ac")) + labs(x = "Coefficent of Variance (%)", title = "Sparkling Lake") + theme(legend.title = element_blank())
...
# Save data
write.csv(trout_cv_stats, file = paste(path2, "Desktop/geodes/intermediate_plotting_files/trout_cv_stats.csv", sep = ""), quote = F, row.names = F)

# Read data back in
trout_cv_stats <- read.csv(file = paste(path2, "Desktop/geodes/intermediate_plotting_files/trout_cv_stats.csv", sep = ""), header = T)


ggplot(trout_cv_stats, aes(x = stat, fill = type)) + geom_density(alpha = 0.5) + scale_fill_manual(values = c("#d8b365", "#5ab4ac", "#99000d")) + labs(x = "Coefficent of Variance (%)", title = "Trout Bog") + theme(legend.title = element_blank())
```
